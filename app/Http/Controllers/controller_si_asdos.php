...
class controller_si_asdos extends Controller
{
    ...
    public function daftar(Request $req)
    {
        if($req->session()->has('role')!=NULL)
        {
            ...
            if($req->session()->get('role')=='2')
            {
                ...
                $nama_mk=$temp->nama_matakuliah($id_slot);
                $tmp1=$temp->insert_data_calon_asdos($nrp,$periode,$id_slot);
                $req->session()->put('result', $tmp1);
                $req->session()->put('nama_makul', $nama_mk->nama_mk);
                $req->session()->put('kelas_makul', $nama_mk->kelas);
                return redirect('mahasiswa/daftar?id='.$nrp.'&status='.$tmp1.'&req_mk='.$nama_mk->idmakul);
                //return redirect('mahasiswa/daftar')->with('result',$tmp1);
                //return redirect('mahasiswa/daftar?status='.$tmp1);
            }
            else
            {
                $message='error please relogin';
    ...
    public function menerima_asdos(Request $req , $id)
    {
        if($req->session()->has('role')!=NULL)
        {
            ...
            {
                $temp=new Asdos();
                $answ=$temp->insert_asdos_baru($id);
                $req->session()->put('flag', $answ);
                //var_dump($ans);
                //return redirect('admin/get_asdos?status='.$ans)->with('result',$ans);
                return redirect('admin/get_asdos?status='.$answ);
            }
            else
            {
                $message='error please relogin';
