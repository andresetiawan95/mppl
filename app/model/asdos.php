...
{
    public function nama_matakuliah($idmk) {
        return DB::table('slot_asdos')
        ->select ('slot_asdos.id_slot_asdos as id', 'slot_asdos.id_mk as idmakul', 'mata_kuliah.nama_mk as nama_mk', 'slot_asdos.kelas as kelas')
        ->join('mata_kuliah','mata_kuliah.id_mk','=','slot_asdos.id_mk')
        ->where('slot_asdos.id_slot_asdos','=',$idmk)
        ->first();
    }
    ...
    public function get_asdos_ada($periode)
    {
        ...
        ->select('calon_asdos.id_calon_asdos as id','member.nama as nama','mata_kuliah.nama_mk as nama_mk','slot_asdos.kelas as kelas','calon_asdos.periode as periode', 'nilai.nilai as nilai')
        ->join('member','member.id_member','=','calon_asdos.id_member')
        ->join('slot_asdos','slot_asdos.id_slot_asdos','=','calon_asdos.id_slot_asdos')
        ->join('mata_kuliah','mata_kuliah.id_mk','=','slot_asdos.id_mk')
        ->join('nilai','nilai.id_member','=','calon_asdos.id_member')
        ->where('mata_kuliah.id_mk','=','nilai.id_mk')
        ->where('calon_asdos.periode','=',"'".$periode."'")
        ->where('calon_asdos.status','=','0')
        ->orderBy('nilai.nilai','desc')
        ->orderBy('mata_kuliah.id_mk','desc')
        ->orderBy('slot_asdos.kelas', 'ASC')
        ->get();*/
        $a=DB::select("(SELECT `calon_asdos`.`id_calon_asdos` AS `id`, `member`.`nama` AS `nama`, `mata_kuliah`.`nama_mk` AS `nama_mk`, `slot_asdos`.`kelas` AS `kelas`, `calon_asdos`.`periode` AS `periode`, `nilai`.`nilai` AS `nilai` FROM `calon_asdos` INNER JOIN `member` ON `member`.`id_member` = `calon_asdos`.`id_member` INNER JOIN `slot_asdos` ON `slot_asdos`.`id_slot_asdos` = `calon_asdos`.`id_slot_asdos` INNER JOIN `mata_kuliah` ON `mata_kuliah`.`id_mk` = `slot_asdos`.`id_mk` INNER JOIN `nilai` ON `nilai`.`id_member` = `calon_asdos`.`id_member` WHERE `mata_kuliah`.`id_mk` = nilai.`id_mk` AND `calon_asdos`.`periode` ="."'".$periode."'"." AND `calon_asdos`.`status` = 0 ORDER BY `nilai`.`nilai` DESC, `mata_kuliah`.`id_mk` DESC, `slot_asdos`.`kelas` ASC)");
        return $a;
    }
